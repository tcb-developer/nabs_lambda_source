...
def fetch_e_proceedings(login_cookies, username, client_name, page_no, download_dir=".", downloaded_files=None, fyi=False, third_party=False, existing_notice_ids=None, password=None):
        ...
        if e_proceedings_group:
            for epro in e_proceedings_group:
                try:
                    ...
                    for notice_idx, notice in enumerate(res_5_data):
                        if notice_idx > 0:
                            time.sleep(1)
                        ...
                        if notice.get('isSubmitted') and notice.get('isSubmitted') == 'Y':
                            try:
                                ...
                                }, cookies=login_cookies.cookies, headers=headers)
                                ...
                                if remark_notice_list:
                                    for remark in remark_notice_list:
                                        for d in remark.get('attachmentLst', None):
                                            ...
                                            # Download file if path provided and not already downloaded
                                            if download_dir and link_text not in downloaded_files:
                                                reply_download_success = False
                                                for dl_attempt in range(1, 4):  # 3 attempts
                                                    try:
                                                        file_res = requests.get(f"{base_url}/document/{d.get('docId')}",
                                                                              headers=headers, timeout=timeout_sec,
                                                                              cookies=login_cookies.cookies)

                                                        # Session expired - refresh and retry
                                                        if file_res.status_code in (401, 403) and password:
                                                            logger.warning(f"Session expired (HTTP {file_res.status_code}) on reply file, refreshing login...")
                                                        ...
                                                        if file_res.status_code != 200:
                                                            logger.warning(f"Reply file download attempt {dl_attempt}/3 failed: {file_name}, Status Code: {file_res.status_code}")
                                                            if dl_attempt < 3:
                                                        ...
                                                        if full_file_path:
                                                            ...
                                                            try:
                                                                ...
                                                                logger.info(f"Reply file uploaded to S3: {file_name}")
                                                            except Exception as s3_error:
                                                                logger.error(f"Error uploading reply file to S3: {s3_error}")
                                                                file_info['s3_error'] = str(s3_error)
                                                        break  # success, exit retry loop

                                                    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                                                        logger.warning(f"Reply file download attempt {dl_attempt}/3 connection error: {file_name}: {e}")
                                                        if dl_attempt < 3:
                                                            time.sleep(dl_attempt * 2)
                                                    except Exception as e:
                                                        logger.error(f"Reply file download unexpected error: {file_name}: {e}")
                                                        break  # don't retry unknown errors

                                                if not reply_download_success:
                                                    logger.error(f"Reply file download FAILED after 3 attempts: {file_name}")
                                            ...
                                            if not file_info['is_acknowledgement']:
                                                notice_document_data['reply_files'].append(file_info)
                                            else:
                                                notice_document_data['response_acknowledgement'] = file_info

                            except requests.exceptions.HTTPError as e:
                                error_msg = f"Not able to download Response Files: {str(e)}"
                                logger.error(error_msg)
                                errors.append({"type": "response_files", "notice": unique_e_pro_notice_id, "error": error_msg})
                            except Exception as e:
                        ...
                        try:
                            ...
                            res_7 = retry_request('post', f"{base_url}/returnservicesapi/auth/saveEntity",
                                json=notice_pdf_payload, headers=headers, cookies=login_cookies.cookies)
                            ...
                            if documents_res_7:
                                for document_id, file_name in documents_res_7.items():
                                    ...
                                    # Download if path provided and not already downloaded
                                    if download_dir and link_text not in downloaded_files:
                                        download_success = False
                                        for dl_attempt in range(1, 4):  # 3 attempts
                                            try:
                                                file_res = requests.get(f"{base_url}/document/{document_id}",
                                                                      headers=headers, timeout=timeout_sec,
                                                                      cookies=login_cookies.cookies)

                                                # Session expired - refresh and retry
                                                if file_res.status_code in (401, 403) and password:
                                                    logger.warning(f"Session expired (HTTP {file_res.status_code}) on notice file, refreshing login...")
                                                    login_cookies = refresh_login_cookies(login_cookies, username, password, client_name)
                                                    continue

                                                # IT portal custom error or server error - retry
                                                if file_res.status_code != 200:
                                                    logger.warning(f"Notice file download attempt {dl_attempt}/3 failed: {file_name}, Status Code: {file_res.status_code}")
                                                    if dl_attempt < 3:
                                                        time.sleep(dl_attempt * 2)
                                                    continue
                                                ...
                                                if full_file_path:
                                                    with open(full_file_path, 'wb') as notice_f:
                                                        notice_f.write(file_res.content)
                                                    logger.info(f"File saved: {file_name}")
                                                    notice_file_info['downloaded'] = True
                                                    download_success = True

                                                    # Upload to S3
                                                    try:
                                                        s3_result = upload_to_s3(full_file_path, link_text)
                                                        notice_file_info['s3_url'] = s3_result.get('file_url')
                                                        notice_file_info['s3_key'] = s3_result.get('content_hash')
                                                        logger.info(f"Notice letter uploaded to S3: {file_name}")
                                                    except Exception as s3_error:
                                                        logger.error(f"Error uploading notice letter to S3: {s3_error}")
                                                        notice_file_info['s3_error'] = str(s3_error)
                                                break  # success, exit retry loop

                                            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                                                logger.warning(f"Notice file download attempt {dl_attempt}/3 connection error: {file_name}: {e}")
                                                if dl_attempt < 3:
                                                    time.sleep(dl_attempt * 2)
                                            except Exception as e:
                                                logger.error(f"Notice file download unexpected error: {file_name}: {e}")
                                                break  # don't retry unknown errors

                                        if not download_success:
                                            logger.error(f"Notice file download FAILED after 3 attempts: {file_name}")
                                            notice_file_info['downloaded'] = False
                                            notice_file_info['error'] = 'Failed after 3 download attempts'
                                    else:
                                        notice_file_info['downloaded'] = False
                                        notice_file_info['skipped'] = True

                                    notice_document_data['notice_letter'] = notice_file_info

                        except requests.exceptions.HTTPError as e:
                            error_msg = f"Not able to download Notice File: {str(e)}"
                            logger.error(error_msg)
                            errors.append({"type": "notice_file", "notice": unique_e_pro_notice_id, "error": error_msg})
                        except Exception as e:
                            error_msg = f"Error downloading notice file: {str(e)}"
                            logger.error(error_msg)
                            errors.append({"type": "notice_file", "notice": unique_e_pro_notice_id, "error": error_msg})

                        # Append each notice as a separate entry
                        e_proceedings_data.append(notice_document_data)
            ...
            if out_file_path:
                ...
                if download_dir and unique_file_name not in downloaded_files:
                    try:
                        ...
                        file_res = requests.post(f"{base_url}/document/order", json={
                            "order": order_id
...
def fetch_income_tax_notices_via_api(username, password, client_name, downloaded_files=None, existing_notice_ids=None):
    """
