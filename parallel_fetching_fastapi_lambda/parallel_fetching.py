...
# 2026-06-02 — this is parallel_fetching_fastapi_lambda, a clone of the
# og_app orchestrator dedicated to the FastAPI backend. og_app's original
# orchestrator (`parallel_fetching_lambda`) stays bit-for-bit unchanged.
# GST worker points to the FastAPI-dedicated GST Lambda. IT worker still
# uses og_app's IT Lambda — IT was outside today's GST-focused work.
# GST worker selection is method-aware (set per-event via `gst_fetch_method`):
#   "selenium" (default) → the browser-based FastAPI GST worker.
#   "api"                → the pure-requests FastAPI GST worker (faster, no
#                          browser). Both workers share the SAME event +
#                          webhook contract, so only the target function name
...
def _gst_lambda_for(method):
    """Resolve the GST worker function name for the requested method."""
    return GST_API_LAMBDA_FUNCTION_NAME if (method or "").lower() == "api" \
        else GST_LAMBDA_FUNCTION_NAME
...
def process_income_tax_client(client):
    ...
    payload = {
        ...
        # Already-fetched notice ids the worker SKIPS (no re-download). The
        # backend builds this onto the client dict; the worker reads it from
    ...
    # the orchestrator from System Configuration).
    if client.get('income_tax_file_download_concurrency'):
...
def process_gst_client(client):
    ...
    # The API worker builds its S3 key as
    # FAPI_GST_API_Notices/<org>/<client>/<ref>/<file>, so it REQUIRES the
    # organization_id. The Selenium worker ignores it. Forward whenever set.
    if client.get('organization_id'):
        payload['organization_id'] = client.get('organization_id')
    if client.get('gstin'):
        payload['gstin'] = client.get('gstin')

    # Forward the file-download concurrency value the worker uses to
    # size its per-case attachment ThreadPoolExecutor. Stamped on the
    # client dict by lambda_handler (above) from the top-level event.
    if client.get('gst_file_download_concurrency'):
        payload['gst_file_download_concurrency'] = client.get(
            'gst_file_download_concurrency'
        )

    # Forward existing-ref-id arrays so the worker can skip already-known
    # notices. Older event payloads omit these; .get() returns None which
    # the worker treats as empty.
    if client.get('existing_ref_ids') is not None:
        payload['existing_ref_ids'] = client.get('existing_ref_ids')
    ...
    # Route to the API worker or the Selenium worker per the client's
    # `gst_fetch_method` (stamped from the top-level event by lambda_handler).
    target_fn = _gst_lambda_for(client.get('gst_fetch_method'))
    return invoke_lambda_async(target_fn, payload, client_info)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Per-invocation GST file-download concurrency override (set by
        # FastAPI System Configuration → "Lambda Parallel File Download
        # Count"). Stamped onto each GST client dict so the per-worker
        # payload carries it through to the worker Lambda, which uses
        # it to size its attachment-download ThreadPoolExecutor.
        gst_file_dl_conc = event.get('gst_file_download_concurrency')
        # Same idea for the IT worker — per-notice file-download concurrency.
        it_file_dl_conc = event.get('income_tax_file_download_concurrency')

        # GST worker selection — "api" (pure-requests worker) or "selenium"
        # (default browser worker). Set once at the top level by FastAPI from
        # its System Configuration "Default Fetch Method"; a per-client value
